...
# Explicitly load from .env file in the project directory
# Get the directory where this script is located
env_path = Path(__file__).parent / '.env'
load_dotenv(dotenv_path=env_path, override=True)

# Configuration - load from .env file (not system environment)
# override=True ensures .env file takes precedence over system environment
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY', '').strip()

# Verify API key is loaded from .env
print("=" * 50)
print("Loading from main.py")
print(f"Loading .env file from: {env_path}")
print(f"✓ OpenAI API key loaded from .env file: {'Present' if OPENAI_API_KEY else 'Missing'}")
if OPENAI_API_KEY:
    print(f"  API Key prefix: {OPENAI_API_KEY[:10]}...")
    print(f"  API Key suffix: ...{OPENAI_API_KEY[-3:]}")
    print(f"  API Key length: {len(OPENAI_API_KEY)} characters")
    print(f"  Expected suffix: ...bUA")
    print(f"  Source: Project .env file (override=True ensures .env takes precedence)")
else:
...
async def handle_media_stream(websocket: WebSocket):
    ...
    # Verify API key is being used for connection
    if OPENAI_API_KEY:
        print(f"Connecting to OpenAI with API key from .env (prefix: {OPENAI_API_KEY[:10]}..., suffix: ...{OPENAI_API_KEY[-3:]})")
    else:
